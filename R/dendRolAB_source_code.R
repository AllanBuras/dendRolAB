...
.PrCoGrAn<-function(DATA,plot.CI=T)
	{
	CI.DATA<-common.interval(DATA,make.plot=plot.CI)
...
pcga<-function(DATA,plot.CI=T)UseMethod("PCGA")

PCGA.default<-function(DATA,plot.CI=T)
	{
	result<-.PrCoGrAn(DATA,plot.CI)
	class(result)<-"PCGA"
	result
	}

plot.PCGA<-function(x,col.vec=NULL,...)
	{
...
btfs<-function(x,y,boot.n=1000)UseMethod("BTFS")

BTFS.default<-function(x,y,boot.n=1000)
	{
	result<-.BTFS(x,y,boot.n=1000)
	class(result)<-"BTFS"
	result
	}

plot.BTFS<-function(x,...)
	{
	par(mfrow=c(1,3))
	for(i in 1:3)
		{
...
sgc<-function(rwl,p.thresh=0.05,make.plot=T,...)UseMethod("SGC")

SGC.default<-function(rwl,p.thresh=0.05,make.plot=T,...)
	{
	result<-.SGC(rwl,p.thresh=0.05,make.plot=T,...)
	class(result)<-"SGC"
	result
	}
